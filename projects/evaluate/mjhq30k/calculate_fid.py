...
def fid_inception_v3():
    ...
    state_dict = torch.load('checkpoints/FID_Inception/pt_inception-2015-12-05-6726825d.pth', map_location='cpu')
...
class DINOv2Detector(torch.nn.Module):
    def __init__(self, feature_dim=1024):
        super().__init__()
        self.feature_dim = feature_dim
        import warnings
        warnings.filterwarnings('ignore', 'xFormers is not available')
        self.model = torch.hub.load('checkpoints/dinov2/facebookresearch_dinov2_main', 'dinov2_vitl14', source='local', pretrained=False)
        self.model.load_state_dict(torch.load('checkpoints/dinov2/dinov2_vitl14_pretrain.pth'))
    ...
    def forward(self, x):
        ...
        # Run DINOv2 model.
        return (self.model.to(x.device)(x).unsqueeze(2).unsqueeze(3),)
...
parser.add_argument(
    "--data", type=str, default='coco', help="Device to use. Like cuda, cuda:0 or cpu"
)
parser.add_argument(
    "--dims",
    type=int,
...
parser.add_argument(
    '--use-dinov2',
    action="store_true",
    help="use dinov2 model to extract features",
    default=False
)
parser.add_argument(
    "path",
...
def get_activations(
    ...
    dataset = ImagePathDataset(files, transforms=TF.Compose([TF.ToTensor(), TF.Resize((299, 299))]))
...
def calculate_metrics_given_paths(paths, batch_size, model, device, dims, num_workers=1):
...
def save_fid_stats(paths, batch_size, model, device, dims, num_workers=1):
...
def main():
    ...
    if args.save_stats:
        save_fid_stats(args.path, args.batch_size, model, device, args.dims, num_workers)
        return


    print('reference: ', args.path[0], 'evaluated: ', args.path[1])
    fid_value = calculate_metrics_given_paths(
        args.path, args.batch_size, model, device, args.dims, num_workers
    )
    print("FID: ", fid_value)
