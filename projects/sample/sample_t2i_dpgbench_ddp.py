...
def main(args):
    ...
    files = os.listdir(args.caption_dir)
    total_samples = int(math.ceil(len(files) / global_batch_size) * global_batch_size)
    pad_num = total_samples - len(files)
    files.extend(files[:pad_num])
    all_captions = []
    for file in files:
        with open(os.path.join(args.caption_dir, file), 'r') as data:
            all_captions.append(data.read())
    all_paths = [os.path.join(sample_folder_dir, file.split('.')[0]+'.png') for file in files]
    ...
    for index in pbar:
        ...
        if args.save_latent:
            ...
            for i, latent in enumerate(batch_latents):
                save_file(dict(latent=latent), paths[i].replace('png', 'safetensors'))
        else:
            batch_images = torch.stack(batch_images, dim=1) 
            B, _, C, H, W = batch_images.shape
            images = batch_images.reshape(B, 2, 2, C, H, W)
            images = images.permute(0, 3, 1, 4, 2, 5).contiguous()  # [B, C, 2, H, 2, W]
            images = images.view(B, C, 2 * H, 2 * W)
            ...
            for i, image in enumerate(images.cpu().numpy()):
                Image.fromarray(image).save(paths[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-type", type=str, default="dpgbench")
